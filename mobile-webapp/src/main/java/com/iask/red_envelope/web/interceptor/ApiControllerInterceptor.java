 ...
 */
@Component
public class ApiControllerInterceptor implements MethodInterceptor {

    private static Logger selfLogger = LoggerFactory.getLogger(ApiControllerInterceptor.class);
    ...
    public Object invoke(MethodInvocation methodInvocation) throws Throwable {
        ...
        try {
            ...
            // AES秘钥是否合法
            String keyValidError = AESEncryption.validateKey(secret);
            ...
            // 响应密文
            String encryptResult = AESEncryption.encrypt(String.valueOf(result).getBytes(RequestConsts.CHARSET), secret);

            // 把返回的密文数据封装成一个JSON对象
            encryptResult = JSON.toJSONString(new EncryptDataVo(encryptResult));
            if (logger != null) {
                logger.setEncryptResult(encryptResult);
            }
