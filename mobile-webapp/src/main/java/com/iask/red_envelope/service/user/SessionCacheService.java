...
public class SessionCacheService {
    ...
    // 设备产商
    public static final String cookieName_cache_session_ma = "ma";

    // 设备系统
    public static final String cookieName_cache_session_os = "os";

    // 设备Id
    public static final String cookieName_cache_session_d = "d";
    ...
    public void updateCachedSession(String sessionId, Map<String, Serializable> sessionMap) {
        try {
//            cacheRedisTemplate_session.setex(sessionId, sessionMap, CacheConsts.SECONDS_OF_HALF_HOUR); // session有效时间将通过配置文件配置
...
//        this.updateCachedSession(newSessionId, sessionMap);
    ...
    public String getRequestedCacheSessionId(HttpServletRequest request) {
        ...
        if (cookies != null && cookies.length > 0) {
            for (Cookie cookie : cookies) {
                if (cookieName_cache_session_id.equals(cookie.getName())) {
                    id = cookie.getValue();
                }else if(cookieName_cache_session_ma.equals(cookie.getName())){
                    ma = cookie.getValue();
                }else if(cookieName_cache_session_os.equals(cookie.getName())){
                    os = cookie.getValue();
                }else if(cookieName_cache_session_d.equals(cookie.getName())){
                    d = cookie.getValue();
                }
            }
        }
        ...
        if (ma != null) {
            request.setAttribute(RequestConsts.ATTR_MANUFACTURER, ma);
        }
        if (os != null) {
            request.setAttribute(RequestConsts.ATTR_OS, os);
        }
        if (d != null) {
            request.setAttribute(RequestConsts.ATTR_DEVICE_ID, d);
        }
        return id;
    }
